...
- **Vanilla comfort settings, applied automatically** — the first time you load this mod, it turns off view bobbing, screen distortion effects, FOV effects, damage tilt, and auto-jump, and raises the field of view to 90 if yours is currently lower (the default is 70; if you're already running something wider, it's left alone). These are Minecraft's own built-in settings, just switched to the values most associated with comfort instead of left at their defaults. Applied once only: if you change any of them back afterward, Steady Sight won't touch them again (this also holds setting-by-setting across mod updates — if a future version adds another setting to this list, only the new one gets pushed, not the ones you already changed back). Toggle this off in the config screen (NeoForge) or config file (Forge) if you'd rather manage these yourself.
...
No sliders, no numbers to guess at — pick a vignette strength, and leave the other toggles on their defaults unless you want to change them. Fabric builds have no config screen or file at all; they run every feature on its default (Standard vignette, everything else on), the same defaults NeoForge and Forge ship with.
