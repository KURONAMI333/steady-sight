 ...
 * hotbar and chat. On 26.2 {@code HudRenderCallback} is gone and
 * {@link HudElementRegistry} replaced it, and it supports insertion relative to
 * a named vanilla element — so the slot the mixin was working around is now an
 * API call, and the mixin is deleted rather than ported.
 *
 ...
 * each vanilla element at its own call site inside {@code Hud#extractRenderState},
 * so this runs immediately after {@code extractCrosshair} — before that method's
 * {@code nextStratum()} and before {@code extractHotbarAndDecorations}, and
 * inside the {@code !isHidden} branch.
 *
