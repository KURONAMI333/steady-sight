 ...
 * 26.2 it is {@code HudElementRegistry.attachElementAfter(
 * VanillaHudElements.CROSSHAIR, ...)}. Both land in the same gap, and on 26.x
 * that gap is also a stratum boundary: vanilla {@code Hud#extractRenderState}
 * calls {@code extractCrosshair} → {@code nextStratum()} →
 * {@code extractHotbarAndDecorations}, so everything drawn after this layer —
 * hotbar, chat, XP bar — is in a later stratum and stays above the mask, which
 * is what picking that slot buys.
 *
...
public final class SteadySightOverlay {
    ...
    public static void render(GuiGraphicsExtractor gui, DeltaTracker tracker) {
        ...
        // 26.x moved the F1 flag: `Options#hideGui` is gone and `Hud#isHidden()`
        // replaced it. The check still has to be here — NeoForge's
        // GuiLayerManager inserts a mod layer without the vanilla group's
        // shouldRender supplier, so a layer that does not check is drawn with
        // the HUD hidden.
     ...
     * (read from the bytecode of {@code minecraft_26.2_client.jar}), whose
     * colour factors are the same multiply, so this cell uses the pipeline
     * and issues no manual {@code RenderSystem} state at all. Per-pixel that
     * is {@code dst_rgb' = dst_rgb * (1 - src_rgb)} with
     ...
     * rather than overwritten with the source's). That is vanilla's own
     * choice on this version and it is strictly the safer one: it is
     ...
     * exact call shape vanilla {@code Hud#extractVignette} uses
     * ({@code ARGB.colorFromFloat(1, f, f, f)} into the 11-argument
     * {@code blit}).
     *
    ...
    private static void drawVignette(GuiGraphicsExtractor gui, Identifier texture, float strength) {
        ...
        // how vanilla's Hud#extractVignette calls this overload — it makes
