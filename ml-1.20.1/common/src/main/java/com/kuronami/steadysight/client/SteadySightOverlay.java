 ...
 * after every camera overlay and immediately before the hotbar. Anything drawn
 * after this layer — hotbar, chat, XP bar — must stay untouched by the mask,
 * which is what picking that slot buys. On Forge 1.20.1 that is
 * {@code registerBelow(VanillaGuiOverlay.HOTBAR.id(), ...)} and on Fabric
 * 1.20.1 a mixin in {@code Gui#render} just before the hotbar branch — note
 * that 1.20.1 draws the hotbar <em>before</em> the crosshair, the reverse of
 * 1.21.1+, so "above the crosshair" is the wrong slot on this version (see
 * this cell's {@code SteadySightForgeClient} and {@code GuiMixin}).
 *
 ...
 * ({@code Dist.CLIENT} event subscriber on Forge, the {@code client}
...
public final class SteadySightOverlay {
    ...
    // 1.20.1 predates ResourceLocation.fromNamespaceAndPath (added in 1.21);
    // the two-argument constructor is still public here.
    private static final ResourceLocation TEXTURE_SUBTLE =
            new ResourceLocation(Constants.MOD_ID, "textures/gui/vignette_subtle.png");
    private static final ResourceLocation TEXTURE_STANDARD =
            new ResourceLocation(Constants.MOD_ID, "textures/gui/vignette_standard.png");
    private static final ResourceLocation TEXTURE_STRONG =
            new ResourceLocation(Constants.MOD_ID, "textures/gui/vignette_strong.png");

    private SteadySightOverlay() {}

    /**
     * {@code partialTick} is unused, exactly as the newer cells'
     * {@code DeltaTracker} parameter is — 1.20.1 has no {@code DeltaTracker}
     * (it arrived in 1.21.1), and this is the value both of this cell's
     * loaders have on hand at the call site.
     */
