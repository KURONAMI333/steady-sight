 ...
 * its own end-of-client-tick hook (Forge {@code TickEvent.ClientTickEvent} at
 * {@code Phase.END} on the Forge bus, Fabric
...
public final class StepCameraTracker {
     ...
     * difference is declared rather than papered over (GAP_LOG G85): Forge
     * calls this from {@code ClientPlayerNetworkEvent}'s LoggingIn, LoggingOut
     * <em>and</em> Clone (the last covers respawn and dimension change — 1.20.1
     * Forge has all three, same as every NeoForge cell), while
     * Fabric 1.20.1 only offers {@code ClientPlayConnectionEvents} JOIN and
