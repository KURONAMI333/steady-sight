...
// Already moved out of `world.entity.vehicle` into a `minecart` subpackage by
// 1.21.11 (verified against minecraft_1.21.11_client_mappings.txt — same
// package split as 26.2, so it happened somewhere between 1.21.1 and here).
// The class and isOnRails() are otherwise unchanged from 1.21.1.
...
public final class StepCameraTracker {
     ...
     * Fabric only offers {@code ClientPlayConnectionEvents} JOIN and
