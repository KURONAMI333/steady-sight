 ...
 * {@code enqueueWork}.
 *
 * <p>Drift from 1.21.1, both mechanical: {@code ResourceLocation} is
 * {@link Identifier}, and {@code @EventBusSubscriber}'s {@code bus} argument
 * is gone — javap on this version's {@code net.neoforged.fml.common.EventBusSubscriber}
 * class (bundled with fancymodloader 10.0.36 / NeoForge 21.11.42) shows only
 * {@code value()} and {@code modid()}, no {@code bus()} at all, so this is
 * already the case here and not something 26.x introduced. The bus is chosen
 * from the event type instead — mod bus for {@code IModBusEvent}, game bus
 * otherwise. The two nested classes are kept as they are; auto-routing would
 * make one class possible, but merging them would be a refactor, not a port.
 *
...
public final class SteadySightNeoForgeClient {
    ...
    public static final class ModBus {
        ...
        /**
         * {@code registerAbove(CROSSHAIR, ...)} inserts right after the
         * vanilla crosshair layer and before the hotbar layer in
         * {@code GuiLayerManager}'s ordered list — the same slot the 1.21.1
         * cell uses. No extract/render-state "stratum" concept exists on this
         * version (that is 26.x-only); {@code GuiLayer.render(GuiGraphics,
         * DeltaTracker)} still draws directly, one layer after another.
         */
        @SubscribeEvent
